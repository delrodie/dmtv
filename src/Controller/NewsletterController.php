...
class NewsletterController extends AbstractController
{
    ...
    public function new(Request $request, NewsletterRepository $newsletterRepository): Response
    {
        ...
        $form->handleRequest($request); //dd($newsletter);

        if ($form->isSubmitted() && $form->isValid()) {
            $email = $newsletterRepository->findOneBy(['email'=> $newsletter->getEmail()]);
            if ($email){
                $this->addFlash('danger', "Echec! Votre email a deja été enregistré pour recevoir les informations");
                return $this->redirectToRoute('newsletter_show',['id'=>$email->getId()]);
            }
